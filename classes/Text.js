class Text {
  ...
  animate() {
    const shift = Math.floor(Math.random(10 - 3) + 3);
    let pos = Math.random().toFixed(1);
    if (pos < 0.1) {
      pos = 0.1;
    } else if (pos > 0.7) {
      pos = 0.7;
    ...
    const newData = this.getPixels(pos);
    const data = Object.assign([], newData.data);
    if (Math.random() < 0.5) {
      for (let i = 0; i < data.length; i++) {
        newData.data[i] = data[
          i - 4 * shift < 0 ? data.length + (i - 4 * shift) : i - 4 * shift
        ];
      }
    } else {
      for (let i = 0; i < data.length; i++) {
        newData.data[i] = data[
          i + 4 * shift < data.length
            ? i + 4 * shift
            : i + 4 * shift - data.length
        ];
      }
